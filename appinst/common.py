...
def _get_install_type():
    ...
    if (all_users_key is not None) and (current_user_key is not None):
        ...
        if os.path.abspath(sys.prefix) == os.path.abspath(all_users_install_path):
            install_type = ALL_USERS
            
        if os.path.abspath(sys.prefix) == os.path.abspath(current_user_install_path):
    ...
    # if we were not able to determine whether python was installed for all users or just
    # the current user, default to the current user    
...
def _get_all_users_quick_launch_directory():
    raise InstallError('Quick Launch is only available for current user installs')
...
def append_to_reg_path( new_dir ) :
    ...
    if _get_install_type() == ALL_USERS:
        ...
        environ_key_path = r"SYSTEM\CurrentControlSet\Control\Session Manager\Environment"
    else:
...
def append_to_reg_pathext():
    ...
    try:
        ...
        if install_type == ALL_USERS:
            ...
            try:
                key = _winreg.OpenKey(hklm_reg, r"SYSTEM\CurrentControlSet\Control\Session Manager\Environment")
                old_pathext = _winreg.QueryValueEx( key, "PATHEXT" )[0]
                _winreg.CloseKey( key )
            except WindowsError:
                old_pathext=""
...
def get_programs_start_menu():
    ...
    """
    if _get_install_type() == ALL_USERS:
        programs_start_menu = _get_all_users_programs_start_menu()
    else:
        programs_start_menu = _get_current_user_programs_start_menu()
